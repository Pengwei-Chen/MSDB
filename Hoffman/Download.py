...
table = "Subset.csv"
prefetch = "/u/home/w/wanluliu/tools/sratoolkit/sratoolkit.2.8.2-1-centos_linux64/bin/prefetch"
sra = "/u/home/w/wanluliu/wanluliu/ncbi/Naive_hESC/sra/"
if sra[-1:] != "/":
    sra += "/"
fastq_dump = "/u/home/w/wanluliu/tools/sratoolkit/sratoolkit.2.8.2-1-centos_linux64/bin/fastq-dump"
...
for sample in scRNA.values():
    for run in sample:
        ...
        file.write(string)
file.close()
print(count, 'scRNA-seq runs detected can be found in "Filtered Records.csv"')
...
for runs_of_a_sample in scRNA.values():
    ...
    for run in runs_of_a_sample:
        ...
        file = open(run[name] + ".sh", "w")
        file.write(command)
        file.close()
        if qsub:
            print("qsub " + run[name] + ".sh")
            print(os.popen("qsub " + run[name] + ".sh").read().rstrip("\n"))
        os.chdir("../")
        print()
    ...
    os.chdir("scripts_outputs_errors")
    file = open("Count_" + run[sample] + ".sh", "w")
    file.write(command)
    file.close()
    os.chdir("../")
    print()